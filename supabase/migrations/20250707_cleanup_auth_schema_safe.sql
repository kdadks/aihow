-- =====================================================
-- SAFE AUTHENTICATION SCHEMA CLEANUP
-- =====================================================
...
BEGIN
    ...
    ELSIF EXISTS (SELECT FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'user_role_assignments') THEN
        CREATE TABLE IF NOT EXISTS temp_role_assignments_backup AS
        SELECT DISTINCT user_id, role_id FROM public.user_role_assignments 
        WHERE user_id IS NOT NULL AND role_id IS NOT NULL;
...
-- Drop user_role_assignments table if it exists (separate DO block)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_role_assignments') THEN
...
-- Drop user_role_assignments view if it exists (separate DO block)
DO $$
BEGIN
...
BEGIN
    ...
    RAISE NOTICE '  - user_role_assignments table';
